								...
								float tx, float ty, float scale, NSVGcachedPaint* cache)
{
	...
	if (cache->type == NSVG_PAINT_COLOR) {
		...
		for (i = 0; i < count; i++) {
			int r,g,b,a,ia;
			// Calculate raw coordinate in gradient space (0=start, 1=end)
			float raw_coord = fx*t[1] + fy*t[3] + t[5];
			float u = raw_coord; // u will be the final coordinate in [0, 1] range

			// Apply spreadMethod logic
			switch (cache->spread) {
				case NSVG_SPREAD_PAD: // Default behavior (clamp)
					u = nsvg__clampf(raw_coord, 0.0f, 1.0f);
					break;
				case NSVG_SPREAD_REPEAT:
					u = fmodf(raw_coord, 1.0f);
					if (u < 0.0f) u += 1.0f; // Ensure positive modulo result
					break;
				case NSVG_SPREAD_REFLECT:
					u = fmodf(raw_coord, 2.0f);
					if (u < 0.0f) u += 2.0f; // Ensure positive modulo result
					if (u >= 1.0f) u = 2.0f - u; // Reflect if in the second half
					break;
			}

			// Get color index from normalized coordinate u
            int index = (int)(u * 255.0f);
            // Ensure index is strictly within bounds [0, 255]
            index = index < 0 ? 0 : (index > 255 ? 255 : index);
			c = cache->colors[index];
		...
		for (i = 0; i < count; i++) {
            ...
            // Calculate raw coordinate (distance from center in gradient space)
			float raw_coord = sqrtf(gx*gx + gy*gy);
            float u = raw_coord; // u will be the final coordinate in [0, 1] range

            // Apply spreadMethod logic (same as linear)
			switch (cache->spread) {
				case NSVG_SPREAD_PAD: // Default behavior (clamp)
					u = nsvg__clampf(raw_coord, 0.0f, 1.0f);
					break;
				case NSVG_SPREAD_REPEAT:
					u = fmodf(raw_coord, 1.0f);
					if (u < 0.0f) u += 1.0f; // Should not happen for radial, but safe
					break;
				case NSVG_SPREAD_REFLECT:
					u = fmodf(raw_coord, 2.0f);
                    if (u < 0.0f) u += 2.0f; // Should not happen for radial, but safe
					if (u >= 1.0f) u = 2.0f - u; // Reflect if in the second half
					break;
			}
